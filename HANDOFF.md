...
- **2026-09-24: AnyList merge field fix (`984ce9c5`).** Live acceptance of Q showed a second recipe's note replacing the first. Bridge 1.7.3's `GET /items` returns the note as `notes`; the merge read `details` (the brief's wrong fact, copied into both the test fake and the local fake bridge) and always saw "". Now read through one helper, `notes` first with `details` as fallback. Lesson: fakes copy the live payload shape, never the brief's description of it. Red-checked: the old read fails 4 tests against the corrected fake; the "ticked re-send" case had been passing by coincidence.

- **2026-09-24: AnyList recipe tag (`63e6715c`).** Every sent item carries a two-line note (recipe name, then `<BASE_URL>/g/<group>/r/<slug>`), built server-side from a recipe resolved by slug in the caller's group; clients send only the slug. A default `http://localhost:8080` BASE_URL counts as unset (name only). `/add` 304s (name already listed) now merge instead of silently no-oping: one `GET /items` per send, append the note unless the recipe URL is already there, always leave the item unchecked. Status per item is `added|merged|failed`; `recipe not found` joins the error taxonomy. The line separator is one constant (`NOTE_SEPARATOR`) in case iOS collapses the newline. Verified against a stateful fake of bridge 1.7.3 and with desktop + mobile screenshots.

- **2026-08-25 — AnyList polish: the pin lives at the backend, the sheet goes opt-in (`9030a656`).** The bridge owns the whole AnyList account, so /lists was passing all 10 lists ("Baby Names", "My Medications") into a shared-household app. New `ANYLIST_LIST` env: when set, /lists answers from config WITHOUT calling the bridge — verified with a call counter that stayed zero for the whole session, so the other names never reach any browser — and /send 400s any other target ("list not allowed", third member of the error taxonomy), which also defuses stale last-used localStorage on any device. Unset = today's generic passthrough, pinned by the existing tests. The frontend keys off "one entry returned" rather than a flag: single-entry mode renders a static "→ name" label, no picker, no last-used memory. The sheet also flips to opt-in — all lines start unchecked, one master header checkbox (indeterminate at partial; note Vuetify signals it via aria-checked="mixed", not the native input property), live "Send N items" count, disabled at zero. Test-infra lesson recorded: v-dialog teleports persist in document.body across vitest cases — unmount + clear per test or later cases query the previous case's stale sheet.

- **2026-08-18 — scaler N.2: glued units, equivalents, ordinals (`7916a5b8`).** The turnovers bug: `30g` didn't scale because the boundary rule protecting `7-Up`/`2% milk` also rejected quantities glued to their unit (16 library rows). Fixed as a whitelist alternation inside the boundary position — glued `g/kg/mg/oz/lb(s)/ml/l/tsp/tbsp/tbs/cup(s)` scales and is preserved as written; everything else keeps the strict rule. Same pass: equivalent-vs-container parentheticals discriminated by one predicate (leading quantity WITH a unit → `(4 Tbsp)` is an equivalent and scales, `60g (4 Tbsp)` → `30g (2 Tbsp)`; bare count → container size stays fixed, `1 (15 oz) can` pinned), and ordinal fractions (`1/6th Batch`) via rational arithmetic so output stays an exact ascii fraction with a recomputed suffix (`1/12th` at ½, `1/3rd` at 2×, whole drops the suffix, unscalable stepper values leave the line untouched). Pure-module change; 33-test pinned table; the wiring was live-verified in PROMPT M.

- **2026-08-18 — Send to AnyList: scaled ingredients onto Mom's shared list (`837e1f4d`).** Fork side only — the bridge (kevdliu's anylist server) deploys separately on CT 100, LAN-internal, credentials in its own env and nowhere else. The browser never talks to the bridge: two authenticated household routes proxy it, with the failure taxonomy designed for diagnosis (env unset → 404 = "not configured", the frontend probe's hide signal; unreachable → fast clean 502; protocol rejection → per-item "AnyList rejected"). The send fans out server-side with per-item results so the sheet can keep failures visible and retry only those. Frontend: an idle-time probe reveals the button (zero residue when disabled); the sheet lists every ingredient line as currently scaled, verbatim — no clever splitting, a wrong parse wastes Mom's time — pre-checked, with a last-used list memory. Verified end-to-end against a fake bridge: 7 half-scaled items landed with the right list, the unchecked line stayed home, and the in-page CORS block on the bridge confirmed only the backend can reach it. No DB writes anywhere.

- **2026-08-18 — cook-mode round: sixteenths, the pinch, and one-tap step timers (`daa8b9e4`, `7ca3dab8`).** Fraction polish first: odd sixteenths render ascii (`1/16`, `1 3/16` — no unicode glyph exists), even ones reduce into the vulgar table, and below 1/16 with a tsp unit the quantity and unit collapse to `pinch` (single values only). Wake-lock was verified working with zero code: the switch stays mounted under cook mode's v-show container, `lockScreen: true` is the default, and VueUse re-acquires after visibilitychange — all confirmed from installed source. Then timers: instruction text like "simmer 15 minutes" grows a tappable chip (recipe page + cook mode, same component) via a duration parser with a REQUIRED time unit (`350 F`/`step 3`/`5 cloves` can never match); ranges time the lower bound. The engine stores target epochs and derives remainders — background throttling cannot drift it, and expired-while-hidden fires on return with "finished Xm ago" (tested across a simulated 10-minute gap). Timer state is module-keyed by slug: survives view↔cook (the sheets are separate instances since the v-if change), dies on page unmount. Done = gold pulse + oscillator chime built inside the start tap (autoplay policy) + vibrate; a bottom strip keeps other steps' timers visible in cook mode. Every chip tap is `@click.stop` — the step card's own click checks steps off. Timers deliberately ignore the scale chips.

- **2026-08-18 — fractional scaling that works on a text-note library (`0c2a2486`).** "Half a sauce" was impossible because upstream scaling multiplies the structured quantity field and the live audit showed all 2,988 ingredient rows are unparsed text notes (qty=0, no food/unit) — scaling had literally never done anything here. The fix is a pure leading-token text scaler (`use-note-scaler`) applied inside `useParsedIngredientText` for unparsed rows only, so recipe page, cook mode, print, and copy-text all scale through the one existing seam with zero surface-specific code. One boundary rule does the safety work (token must be followed by whitespace/`(`/end): `2% milk`, `7-Up`, `cut into 8 pieces`, and `(15 oz)` are untouchable without a denylist. scale=1 early-returns the byte-identical input, so the default path is provably unchanged. UI is a ¼/½/1×/2× chip group (gold active, the rating precedent) in the RecipePageScale surface — present in cook mode too; "Makes N" follows the scale, the per-serving macro cells deliberately don't. Verified live end-to-end on a seeded text-note sauce including exact-string restoration at 1×. 17 scaler tests; display-only, no API writes.

- **2026-08-10 — the launch API storm, and the fallback that pointed at nothing (`83dfcf6c`).** Round 3 persisted caches but the access log showed the real launch cost: six unbounded `perPage=-1` fetches per cold start (organizers ×3, members, households, cookbooks) plus a mount-fired `page=3` recipes fetch, plus the launch navigation itself 404ing at the server. Sources were mapped one by one: the per-card category menu, the explorer search + filter bar (whose option lists actually come from facets — the stores are fallback/deep-link only), and UserAvatar fetching the entire member list for a tooltip the sidebar disables. All now `{ lazy: true }` (the food-store pattern generalized) hydrating on open/hover/reference; households hydrates at idle because its filter's visibility reads store rows. The infinite-scroll handler now honors `isIntersecting` (v-intersect fires on mount). The 404: `navigateFallback: "/"` pointed at a shell round 2's globIgnores had excluded from the precache — and `yarn build` emits no html so it looked harmless locally; only the image's `yarn generate` does. index.html is precached again (`{url:"/"}` verified in the generate sw.js). Access-log verified: launch 13 → 9 requests, zero unbounded on the paint path.

- **2026-08-10 — the standalone-only lag: warmth now survives process death (`8d631d1f`).** Ethan's "browser tab is snappy, installed app feels bad" was measured for the first time in the configuration he meant: CDP attached to the real Brave WebappActivity on the Fold (no WebAPK exists — Brave registers webapps internally; launch = force-stop + home-icon tap, page 2 (791,1806); `WebappActivity` is not exported so `am start` is denied, and emulating display-mode doesn't reproduce anything). The trace acquitted the round-2 precache trade and SW boot (all 245 chunks cache-served, workerStart 3ms) and convicted process death: the SWR list cache and cookbook store are module-scoped, and the installed app cold-starts a fresh renderer every launch — drawer cookbook section absent until ~1,070ms, first cookbook tap skeletoned until ~1,029ms with a 345ms grid-mount long task, every single launch. Both caches now persist to localStorage and re-seed on launch, gated on the session confirming the same user id, cleared on sign-out/mutation, public-explorer data excluded; verified live: post-reload cold tap timeline `[{t:0, cards(9)}]`, zero skeleton frames. `standaloneColdProbe()` in nav-probe.mjs carries the budgets. Known remaining: the 345ms 64-card mount (virtualization, future brief). New probe trap recorded: the 250-entry resource-timing buffer fills with cached chunks and silently hides late API calls.
